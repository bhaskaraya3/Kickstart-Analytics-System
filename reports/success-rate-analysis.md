...
### Result
| state      | total_projects | percentage |
|------------|---------------|------------|
| Failed     | 197611        | 59.62%     |
...
category,
ROUND(
	SUM(CASE WHEN state='Successful' THEN 1 ELSE 0 END)*100.0/COUNT (*),2
) AS success_rate
FROM kickstart
GROUP BY category
...
2. *Minimal backers.*
3. *Mostly Failed, Suspended, or Canceled state.s*
...
SELECT
    ...
    ROUND(
        SUM(CASE WHEN state = 'Failed' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2
    ) AS failed_rate_pct
FROM kickstart
...
## Result
| Duration Bucket | Success Rate | Failure Rate |
|-----------------|-------------|--------------|
| Short (0–30)    | 40.88%      | 59.12%       |
| Medium (31–60)  | 39.53%      | 60.47%       |
| Long (61+)      | 38.74%      | 61.26%       |
